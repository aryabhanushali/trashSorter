...
Original file is located at
    https://colab.research.google.com/drive/1S_wuh1RJPbjrqGu0ewd2mykXrYZJC3k0
...
np.random.seed(3630) #This sets the seed of the random number generator to 3630. It ensures that every time you run the code, the random sampling will behave in exactly the same way.
unit_test = TestProject1()

"""**Useful Global Variables:**"""

#export
class Trash(Enum): #assigns a unique integer to each type of trash.
    CARDBOARD = 0
    PAPER = 1
    CAN = 2
    SCRAP_METAL = 3
...
class DiscreteDistribution:
  def __init__(self, prior_names, prior):
    ...
    '''
    self._names = prior_names.copy() # Copy the list of category names
    self._prior = np.array(prior, dtype=float) # Convert prior probabilities to a NumPy array
    self._prior /= self._prior.sum() # Normalize the probabilities to ensure they sum to 1

  def get_name_index(self, name) -> int:
    '''
    Helper function to get index of prior name
    '''
    #This function allows you to get the index of a category by its name. For example, if you pass in "cardboard", it will return the index of "cardboard" from the list self._names.
    return self._names.index(name)

  def pmf(self) -> list:
    return self._prior
#PMF (Probability Mass Function): This method returns the current probabilities stored in self._prior. It essentially returns the list of probabilities that corresponds to the likelihood of each category.
#. The PMF provides the probabilities of each possible outcome in the discrete distribution.

  # TODO 1:
  # sample item
  def sample(self) -> int: #generates a random sample based on the discrete probability distribution defined by the _prior probabilities.
  #The result is an index that corresponds to one of the categories in the _names list.
    '''
    ...
    '''
    #First Parameter (len(self._prior)): Specifies the number of possible choices
    #second param: array where each entry is the probability of the corresponding index being chosen.

    ###############################################################################
    #                             START OF YOUR CODE                              #
    ###############################################################################
    name = np.random.choice(self._names, p=self._prior)
    sampled_index = self.get_name_index(name)
...
def get_category_prior():
    ...
    #create and return an instance of DiscreteDistribution w/ the prior probabilities for various trash categories
...
def sample_category():
    ...
    ###############################################################################
    category_prior = get_category_prior()
    #get_category_prior function to get a DiscreteDistribution object that holds the prior probabilities of different trash categories.
    sample = category_prior.sample() #sample method is expected to return an integer index representing one of the trash categories.
...
class DiscreteConditional:
  def __init__(self, A_names, B_names, cpt):
    ...
    '''
    self._B_names = B_names.copy() # values or categories for the variable B
    self._A_names = A_names.copy() # values or categories for the variable A
    self._cpt = np.array(cpt, dtype=float) #Converts the CPT into a NumPy array for efficient mathematical operations.
    self._cpt /= np.sum(self._cpt, axis=1)[:, np.newaxis] #normalizes 
  ...
  def sample(self, B_index: int) -> int:
    ...
    #                             START OF YOUR CODE                              #
    ###############################################################################
    probs_A_given_B = self._cpt[B_index]
    sampled_index = np.random.choice(len(self._A_names), p=probs_A_given_B)
...
class GaussianConditional:
    ...
    def Gaussian(x, mu=0.0, sigma=1.0):
      ...
      #computes the probability density function (PDF) of a Gaussian (normal) distribution for a given value x, mean mu, and standard deviation sigma
    ...
    def sample(self, B_index):
        ...
        '''
        mu = self._means[B_index] #retrieves the mean value mu for the category B_index
        sigma = self._sigmas[B_index] #Retrieves the standard deviation sigma for the category B_index


        weight = np.random.normal(mu, sigma) #sample weight from a Gaussian distribution with the mean mu and standard deviation sigma
        ###############################################################################
        #                             START OF YOUR CODE                              #
        ###############################################################################
...
def get_pCT():
    ...
    #                             START OF YOUR CODE                              #
    ###############################################################################
    prob_distribution= [0.99, 0.01],[0.99, 0.01],[0.10, 0.90], [0.15, 0.85],[0.95, 0.05]
...
def get_pDT():
    ...
    #                             START OF YOUR CODE                              #
    ###############################################################################
    prob_distribution = [[0.02, 0.88, 0.1], [0.02, 0.2, 0.78], [0.33, 0.33, 0.34], [0.33, 0.33, 0.34], [0.95, 0.02, 0.03]]
...
def sample_detection(category=None):
    ...
    if category is None:
      ...
      detection_conditional_distribution =  get_pDT()
      detection = detection_conditional_distribution.sample(category)
...
def bayes_given_weight(weight):
   ...
   # posteriors = None
    ...
    #                             START OF YOUR CODE                              #
    ###############################################################################
    priors = np.array(get_category_prior_pmf())

    pWT = get_pWT()
    likelihoods = pWT.likelihoods(weight)

    marg = np.sum(likelihoods * priors)


    posteriors = (likelihoods * priors) / marg
...
def bayes_given_three_sensors(conductivity, detection, weight):
    ...
    #                             START OF YOUR CODE                              #
    ###############################################################################
    approx = np.array(Appromixation) / np.sum(Appromixation)
    likelihoods = likelihood_given_detection(detection) * likelihood_given_weight(weight) * get_pCT().likelihoods(conductivity)
    temp = likelihoods * approx
    posteriors = temp/np.sum(temp)
...
def make_decision(posteriors):
    ...
    #                             START OF YOUR CODE                              #
    ###############################################################################
    action = np.argmin(np.dot(COST_TABLE, posteriors))
